...
char telldir(int b)// depending on value of z it gives the direction     //doubt
{
  if(b==4)
  {
return 'j';    
...
int loopval()// it returns the type of loop, 1st type if without opening and 2nd type with opening in between
{
  for(int x=ind;x>=0;x--)
  {
    ...
    if(sumval<=5)// allowance of error of 5 cm
    {
      ...
      if(ci==cj)// check same directions in loop                               //doubt
      {
        for(y=ind;y>=x;y--)
        {
          if(indpt[y]>0)
